...
type Options struct {
	...
	CompanyPrefix          string        `json:"companyPrefix"`       // Prefix added to each helm deployment name.
	...
	GitRepo                string        `json:"gitRepo"`             // Name of the remote git repo where helm charts are served.
...
func (o *Options) FillConfig() {
	...
	o.CompanyPrefix = viper.GetString("company_prefix")
	o.GitRepo = viper.GetString("git_repo")
	o.HealthRoute = viper.GetString("health_route")
	o.Hostname = viper.GetString("host_name")
	o.ImageTagPrefix = viper.GetString("image_tag_prefix")
